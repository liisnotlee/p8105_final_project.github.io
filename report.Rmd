...
ggplot(cycle_avg, aes(x = phase, y = avg_appetite, group = 1)) +
  ...
  geom_text(aes(label = round(avg_appetite, 2)),
            vjust = -0.8, size = 4, fontface = "bold") +
  
  # Add more space above labels
  coord_cartesian(clip = "off") +
  theme(plot.margin = margin(t = 20)) +
  
  labs(
    title = "Average Appetite Across Phases",
...
Appetite was highest in the Fertility phase (2.96), a pattern that visually paralleled the estrogen trend, leading me to formally test their relationship using a linear mixed model.
...
Anu: Several limitations are to be acknowledged. The observational design precludes causal inference, and unmeasured confounders may influence both activity and symptoms. Activity was predominantly walking (70-85% of exercise days), limiting generalizability to structured or higher-intensity exercise programs. Incomplete symptom reporting resulted in substantial missing data (many observations recorded as N/A), reducing effective sample sizes and potentially introducing selection bias. Same-day measurement may miss lagged or cumulative effects of sustained exercise. Symptom scales were ordinal (0-5 Likert) rather than truly continuous. While ordinal mixed models would be theoretically more appropriate, linear models are standard practice for Likert scales with 6+ levels and provide more interpretable estimates. High individual heterogeneity in exercise-symptom relationships may obscure subgroup effects averaged in population-level models. 
Khue : The sample size and ethnic distribution may limit generalizability; some ethnic groups were represented by fewer participants, potentially affecting the stability of subgroup trends. In addition, leep and stress scores were derived from self-reported or device-based composite measures, which may not capture the full complexity of physiological processes such as sleep architecture or cortisol variation. 
