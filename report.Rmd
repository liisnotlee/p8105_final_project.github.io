...
## Related Work
Our project is inspired by new and upcoming research examining how lifestyle factors influence menstrual health and the emerging use of wearables in this space.

Studies have established that short sleep duration and both sedentary and vigorous physical activity can contribute to the risk of developing menstrual disorders [(PubMed Central)](https://pmc.ncbi.nlm.nih.gov/articles/PMC10393940/), while poorer sleep quality in the premenstrual phase and during menstruation is common in women with premenstrual symptoms or painful cramps [(PubMed)](https://pubmed.ncbi.nlm.nih.gov/30098748/).  Regarding exercise, it has been found that exercise performed for about 45 to 60 minutes, three times per week or more, may provide a clinically significant reduction in menstrual pain intensity [(PubMed)](https://pubmed.ncbi.nlm.nih.gov/31538328/).

<br>

## Initial Questions

1. What are the demographic and health characteristics of participants in this study?
2. How do hormone fluctuations across the cycle correlate with reported symptoms?
3. How do sleep quality and stress levels vary across menstrual cycle phases, and do hormonal changes explain these patterns?
4. Does physical activity level influence the severity of menstrual symptoms such as pain, bloating, and fatigue?
5. Can we use wearable data and self-reported information to predict menstrual symptom severity?
...
bmi_all <- subject_info |> 
  filter(!is.na(bmi)) |> 
  ggplot(aes(x = "All participants", y = bmi, fill="bmi")) +
...
We looked at average appetite to observe general trends across the menstrual cycle. Average appetite score was highest in the fertility phase (2.96), a pattern that visually paralleled the estrogen trend we saw earlier. This warrants formal statistical testing, where we can investigate the relationship between the two (using a linear mixed model).
...
mood_plot1 =
  ...
  labs(
    title = "Mood Swing Intensity Distribution",
    x = "Phase",
...
Here, we assess flow volume distribution patterns, and visualize flow volume variation across phases with violin plots.
...
flow_plot1 =
  ...
  geom_jitter(width = 0.15, alpha = 0.3, size = 1) +
  labs(
    title = "Flow Volume Distribution",
...
flow_plot2 =
  ...
  labs(title = "Average Flow Volume",
...
We explored the variation of stress score based on several variables including the menstrual phaes, the time in bed, deep sleep, moodswing score and total exercise minutes. 
With an estimate of 0.032 and a p< 0.05, deep_sleep_in_minutes is a significant positive predictor of stress score. This suggests that, on average, for every additional minute of deep sleep, the stress score is predicted to increase by 0.032 points, assuming other variables are constant. Time in bed is a is a significant positive predictor of stress score ( p <0.05) For every additional minute in bed (mean), the stress score is predicted to increase by 0.009 points, holding all other variables constant. The 95% confidence intervals for the significant predictors (deep_sleep_in_minutes and timeinbed_mean) do not contain zero, further supporting their statistical significance. When we explore more variables, the menstrual phases do not affect the stress score significantly. But we can see the strong association between stress and sleep. 
