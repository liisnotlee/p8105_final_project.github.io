---
title: "EXERCISE & MENSTRUAL HEALTH"
...
overall_desc = all_daily_data |> 
  summarise(
    ...
    Percent = c(NA, NA, NA, NA,
                mean(exercised_flag > 0, na.rm = TRUE) * 100)
  ) |> 
  mutate(
...
```
Overall, participants demonstrated moderate daily physical activity levels. The median daily step count was `r format(round(overall_desc$median_steps, 0), big.mark = ",")` steps, with a mean of `r format(round(overall_desc$mean_steps, 1), big.mark = ",")` steps (SD = `r format(round(overall_desc$sd_steps, 1), big.mark = ",")`). The large standard deviation and difference between mean and median indicate presence of extreme outlier observations (as identified earlier). Active exercise time averaged `r round(overall_desc$mean_active, 1)` minutes per day (Median = `r round(overall_desc$median_active, 1)` minutes), with participants burning an average of `r format(round(overall_desc$mean_calories, 1), big.mark = ",")` calories per session (Median = `r format(round(overall_desc$median_calories, 0), big.mark = ",")`). Most participants engaged in `r round(overall_desc$mean_sessions, 1)` exercise sessions per day (Median = `r round(overall_desc$median_sessions, 1)`). Physical activity was recorded on `r round(overall_desc$pct_active, 1)`% of study days, indicating high compliance with activity tracking.
...
year_comp = all_daily_data |> 
  ...
  summarise(
    ...
    `% Days Active` = round(mean(exercised_flag, na.rm = TRUE) * 100, 1)
  ) |> 
...
kable(year_comp,
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
Exercise patterns differed substantially between study years. The 2024 cohort showed considerably higher activity levels compared to 2022 across all metrics. 2024 participants averaged `r round(year_2024$mean_active, 1)` active minutes per day compared to `r round(year_2022$mean_active, 1)` minutes in 2022. The proportion of days with recorded activity increased from `r round(year_2022$pct_active, 1)`% in 2022 to `r round(year_2024$pct_active, 1)`% in 2024.
...
kable(symptom_desc,
      ...
      caption = "Descriptive Statistics: Symptom Severity (Scale 0-5)",
      align = c("l", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
Symptom severity varied across menstrual cycle phases and between individuals. Most physical symptoms showed low-to-moderate mean severity on the 0-5 scale: cramps (M = 1.00, SD = 1.27), fatigue (M = 2.50, SD = 1.48), and bloating (M = 1.60, SD = 1.48). Psychological symptoms including mood swings (M = 1.57, SD = 1.42) and appetite changes 
(M = 2.91, SD = 0.83) were similarly mild on average. The relatively large standard deviations across all symptoms reflect substantial variability in symptom  experiences across the menstrual cycle.
